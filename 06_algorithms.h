...
private:
	...
	static constexpr OutIter __default_cpoy(Iterator first, Sentinel last, OutIter result) noexcept
	{
		...
		using remove_cvref_iter_t = _STD remove_cvref_t<Iterator>;
		using remove_cvref_out_t  = _STD  remove_cvref_t<OutIter>;
		using remove_cv_iter_t	  = _STD	remove_cv_t<Iterator>;

		if constexpr ((_STD is_same_v<remove_cvref_iter_t, remove_cvref_out_t>)&&(
						  (_STD is_same_v<remove_cv_iter_t,
