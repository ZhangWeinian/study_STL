...
namespace zhang::algorithms
{
	...
	namespace namespace_function
	{
		...
		inline auto count(InputIterator first, InputIterator last, const T& value)
			-> __difference_type_for_iter<InputIterator>
		{
			return namespace_function::count(
				first,
				last,
				value,
#ifdef _RANGES
		...
		{
			return namespace_function::find(
				first,
				last,
				value,
#ifdef _RANGES
...
#endif // __HAS_CPP20
		{
			return namespace_function::find_first_of(
				first1,
				last1,
				first2,
				last2,
		...
		{
			return namespace_function::equal(
				first1,
				last1,
				first2,
#ifdef _RANGES
		...
		{
			return namespace_function::max_element(
				first,
				last,
#ifdef _RANGES
				_RANGES less {}
		...
		{
			return namespace_function::min_element(
				first,
				last,
#ifdef _RANGES
				_RANGES greater {}
		...
		{
			return namespace_function::merge(
				first1,
				last2,
				first2,
				last2,
	...
	namespace namespace_sort
	{
		...
		namespace std_sort
		{
											  ...
											  Function			   fun) noexcept
			{
				...
				__zh_heap sort_heap(first, middle, fun);
			}
		...
		namespace merge_sort
		{
			...
			*/
			...
			inline void merge_sort(BidirectionalIterator first, BidirectionalIterator last, Function fun) noexcept
			{
				...
				difference_type n	  = _STD_RANGES distance(first, last);
		...
		namespace heap_sort
		{
			...
			{
				namespace_sort::std_sort::__heap_sort(
					first,
					middle,
					last,
#ifdef _RANGES
	...
	namespace namespace_binary_search
	{
		...
		inline ForwardIterator upper_bound(ForwardIterator first, ForwardIterator last, const T& value, Function fun)
		{
			...
			{
				len = _STD_RANGES distance(first, last);
			}
			...
			while (len > 0)
			{
				...
				{
					middle = first;
					_STD_RANGES advance(middle, half);
				}
		...
		{
			return namespace_binary_search::upper_bound(
				first,
				last,
				value,
#ifdef _RANGES
		...
		{
			return namespace_binary_search::lower_bound(
				first,
				last,
				value,
#ifdef _RANGES
						 ...
						 equal_range(ForwardIterator first, ForwardIterator last, const T& value, Function fun)
		{
			...
			while (0 < len)							 // 如果整个区间尚未迭代完毕
			{
				...
				else						  // 如果 中央元素 == 指定值
				{
					...
					return __zh_pair pair<ForwardIterator, ForwardIterator>(__move(left), __move(right));
				}
			}

			// 整个区间内没有匹配值，则返回一对迭代器--指向第一个 大于指定值 的元素
			return __zh_pair pair<ForwardIterator, ForwardIterator>(__move(first), __move(first));
		}
		...
		{
			return namespace_binary_search::binary_search(
				first,
				last,
				value,
#ifdef _RANGES
